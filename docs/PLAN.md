...
| 친구 기능 범위 | **MVP에 포함** |
| 마스코트 시스템 | **함수형 식물 (procedural)** — 씨앗 + 영양분(공부/운동 분) → PlantFormula 결정론적 함수. AI 에셋 폐기 |
...
| `2.jpeg` (Grennify) | 톤 | 둥근 카드, 곡선 프레임, 식물 일러스트 → 마스코트 배경 곡선에 적용 |
...
| 마스코트 성장 | ✅ | **함수형 식물** — `PlantFormula(seed, studyMinutes, workoutMinutes)`. 사람마다 다른 seed → 다른 모양. 식 자체를 사용자가 PlantDetailView 에서 확인 가능 |
| 통계 (일/주/월) | ✅ | |
...
| ~~AI 마스코트 일러스트 품질~~ | (폐기) | 사용자 지시로 함수형 식물 procedural 생성으로 전환. AI 에셋 미사용 |
