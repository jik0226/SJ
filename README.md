...
> 종이 플래너의 10분 블록 그리드를 디지털로 옮기고, **수학 함수로 출렁이는 바다 마스코트**가 학습·운동 시간을 영양분으로 받아 파도·물고기로 표현되는 iOS 학습 동반자 앱.
>
...
| **함수형 바다 마스코트** | (씨앗 + 학습 분 + 운동 분) → sin 파동 다층 파도 + 결정론적 물고기 떼 — **AI 에셋 없이 수학으로만 사람마다 다른 바다** |
| **연속 학습 streak** | UserDefaults 영속 7일 streak, 도달 시 바다 영양분 보너스 + 알림 |
| **위젯 + 위젯 자동 동기화** | 디데이 / 오늘 순공 / 내 바다 3종, App Group 으로 데이터 공유 |
...
```
파도 레이어 수 = min(4, 1 + ⌊log₁₀(총분 + 1) × 1.5⌋)
파도 진폭     = 4 + √(운동 분 ÷ 20) + 레이어 인덱스 × 1.5
파도 진동수   = 0.4 + ln(공부 분 + 1) × 0.08 + 레이어 인덱스 × 0.15
파도 위상     = seed 기반 결정론적 jitter (사용자별 고유)
물고기 수     = min(12, max(0, ⌊log₁₀(총분 + 1) × 3⌋ − 1))
배경 색조     = 0.58 − 공부비율 × 0.05  (공부 많을수록 차가운 새벽 톤)
```

같은 식이라도 **사용자별 seed 가 다르므로 파도·물고기 배치가 모두 다르고**, 본인의 식 자체를 detail 화면(`내 바다의 식`)에서 직접 확인할 수 있음. SwiftUI Canvas 로 sin 파동·결정론적 fish 마크만 그려 외부 에셋 0.
...
│                     DailyPage / DDay / Mascot (legacy) / PausedRange
...
├── MascotEngine      진화 룰 (legacy, 바다 시스템으로 대체)
└── PlantFormula      seed + 영양분 → 파도·물고기 OceanParameters 결정론적 함수

StudyApp (iOS Application)
├── App               StudyApp @main, AppState (@Observable, MainActor)
├── Features          Home / Timer (포모도로 다이얼 포함) / Planner / DDay /
│                     Subject / Plant (바다 디테일) / Running / Stats /
│                     Groups / Friends / Onboarding
...
│                     PlantProgressService (바다 영양분) /
│                     StreakService / WidgetSyncService
...
├── DDayWidget · TodayStudyTimeWidget · PlantWidget (내 바다) · TimerLiveActivity
```
...
| W5 | 마스코트 → **함수형 바다 (파도 + 물고기)** 전면 재설계 (사용자 피드백 반영) |
...
- **함수형 마스코트**: 외부 에셋 의존도를 0 으로 만드는 procedural 접근. 사용자가 자기 바다의 식(파도 진폭/주파수/물고기 수)을 직접 보는 투명성 + 사람마다 다른 시드로 unique 경험
